    ...
    public boolean onKeyDown(@NotNull Level level, @NotNull Entity interactEntity, int key, int scanCode, int action) {
        ...
        if (player.isSteppingCarefully() && ManaitaKey.SwitchExterminationKey.matches(key, scanCode)) {
            ...
            if (data != null) {
                ...
                player.displayClientMessage(Component.literal(ManaitaDataComponents.STRIKER_TEXT.getString() + ": " + (data.getMsg() ? ManaitaDataComponents.IS_KILL_ALL : ManaitaDataComponents.ONLY_ENEMY).getString()), true);
            }
        }
        return true;
    }
